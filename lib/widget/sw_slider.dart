...
class _SliderState extends State<SWSlider> with TickerProviderStateMixin {
...
//    positionController.animateTo(0.5);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.activeColor != null || widget.inactiveColor != null) {
      sliderTheme = sliderTheme.copyWith(
        ...
        disabledActiveTrackColor: widget.bgColor ?? widget.inactiveColor,
      );
    }
...
class _RenderSlider extends RenderBox {
  ...
  static const double _overlayRadius = 6.0;
  ...
  set value(double newValue) {
    ...
    if (!isDrag && !isEditDarg) {
      _state.positionController.value = newValue;

      if (newValue > 0 && newValue < 0.125) {
        convertedValue = 0;
      } else if (newValue > 0.125 && newValue < 0.375) {
        convertedValue = 0.25;
      } else if (newValue > 0.375 && newValue < 0.625) {
        convertedValue = 0.5;
      ...
      Future.delayed(Duration(milliseconds: 300)).then((_) {
        ...
        final double distance =
            (_value - _state.positionController.value).abs();
        _state.positionController.duration = distance != 0.0
            ? _positionAnimationDuration * (2.0 / distance)
            : Duration.zero;
        _state.positionController
            .animateTo(convertedValue, curve: Curves.easeOut);
        _value = convertedValue;
        onChanged(_value);
  ...
  void _paintTickMarks(
    ...
    if (isDiscrete) {
      ...
      for (int i = 0; i < 5; i += 1) {
        ...
        double offset = trackLeft.left + offsetValue;

        final double left = i * (trackWidth - trackLeft.left - 2) / 4 + offset;

        //两边小方块
        var leftMaskRect = Rect.fromLTRB(
            left - 8, trackLeft.top - 5, left - 4, trackLeft.top + 5);
        var rightMaskRect = Rect.fromLTRB(
            left + 4, trackLeft.top - 5, left + 9, trackLeft.top + 5);
        canvas.drawRect(leftMaskRect,
            Paint()..color = _sliderTheme.disabledActiveTrackColor);
        canvas.drawRect(rightMaskRect,
            Paint()..color = _sliderTheme.disabledActiveTrackColor);

        if (left > trackRight.left) {
          paint = leftPaint;
        } else {
          paint = rightPaint;
        }
        ...
        //小菱形块
        Path path = Path()..moveTo(left - 5 + offsetLeft, trackLeft.top + 1.2);
        path.lineTo(left + 1, trackLeft.top - 5);
        path.lineTo(left + 7 + offsetRight, trackLeft.top + 1);
        path.lineTo(left + 1, trackLeft.top + 7);
        canvas.drawPath(path, paint);
      }
    }
  }

  void _paintOverlay(Canvas canvas, Offset center, Paint leftTickMarkPaint) {
//    if (!_overlayAnimation.isDismissed) {
    ...
    final Paint overlayPaint = Paint()..color = Colors.blue;
    final double radius = _overlayRadiusTween.evaluate(_overlayAnimation);
//      canvas.drawCircle(center, Adaptive.width(12), overlayPaint);
    canvas.drawRect(
        Rect.fromLTRB(
            center.dx - 7, center.dy - 10, center.dx + 7, center.dy + 10),
        leftTickMarkPaint);
...
//    final double trackLeft = offset.dx + _overlayRadius;
    ...
    final double trackTop =
        trackVerticalCenter - trackRadius - Adaptive.width(0.5);
    ...
    final double thumbRadius = _sliderTheme.thumbShape
            ...
            .width /
        2.0;
    final double trackActiveLeft =
        math.max(0.0, trackActive - thumbGap * (1.0 - _enableAnimation.value));

    final double trackActiveRight = math.min(
        trackActive + thumbGap * (1.0 - _enableAnimation.value), trackRight);
    ...
    final Rect trackRightLineRect =
        Rect.fromLTRB(trackActiveRight, trackTop, trackRight - 5, trackBottom);
    ...
    if (visualPosition < 1.0) {
      canvas.drawRect(trackRightLineRect, rightTrackPaint);
    }
    ...
    _paintTickMarks(
      ...
      leftTickMarkPaint,
      rightTickMarkPaint,
      trackRightRect.right,
    );

    double thumbOffset = 0;
    if (visualPosition < 0.1) {
      thumbOffset = 6;
    } else if (visualPosition > 0.9) {
      thumbOffset = -6;
    }

    final Offset thumbCenter =
        Offset(trackActive + thumbOffset, trackVerticalCenter);

    _paintOverlay(canvas, thumbCenter, rightTickMarkPaint);
