...
import 'package:flutter/material.dart'
    hide showMenu, PopupMenuEntry, PopupMenuButton;
...
const double _kMenuDividerHeight = 1.0;
...
double _kMenuScreenPadding = 0.0;
Color _menuBgColor = Color(0xFF393939);
EdgeInsetsGeometry anchorRightPadding;
...
class PopupMenuConfig {
  ...
  static void config({
    double menuScreenPadding = 0.0,
    Color menuBgColor = const Color(0xFF333333),
  }) {
    _kMenuScreenPadding = menuScreenPadding;
...
class _SWPopupMenuDividerState extends State<SWPopupMenuDivider> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Divider(color: widget.color, height: widget.height),
    );
  }
}
...
class _PopupMenu<T> extends StatelessWidget {
  const _PopupMenu({
    ...
    this.route,
    this.semanticLabel,
    this.anchorRightPadding,
  }) : super(key: key);

  final _PopupMenuRoute<T> route;
  final String semanticLabel;
  final EdgeInsetsGeometry anchorRightPadding;
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> menuChildren = <Widget>[];
    if(anchorRightPadding != null){
    ...
    menuChildren.add(Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _menuBgColor),
      ),
...
class _PopupMenuRoute<T> extends PopupRoute<T> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    Widget menu = _PopupMenu<T>(
        ...
        anchorRightPadding: anchorRightPadding);
...
class _PopupMenuButtonState<T> extends State<SWPopupMenuButton<T>> {
  void showButtonMenu() {
    ...
    showMenu<T>(
      ...
      position: position,
      anchorRightPadding: widget.anchorRightPadding,
...
class _AnchorWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ClipPath(
      ...
      clipper: _AnchorClipper(anchorContext: key.currentContext),
    );
  }
}

///
class _AnchorClipper extends CustomClipper<Path> {
  final double anchorMiddleX;
  final double anchorWidth = 15;
  final double anchorHeight = 10.0;
  final double rawWidth;
  final double rawHeight;
  final BuildContext anchorContext;

  _AnchorClipper(
      {this.anchorContext, this.rawHeight, this.rawWidth, this.anchorMiddleX});

  @override
  getClip(Size size) {
//    return Path()
//      ..moveTo(rawWidth - anchorMiddleX, 0.0)
//      ..lineTo(rawWidth - anchorMiddleX + anchorWidth / 2, anchorHeight)
//      ..lineTo(rawWidth - anchorMiddleX - anchorWidth / 2, anchorHeight)
//      ..lineTo(rawWidth - anchorMiddleX, 0.0)
//      ..close();
//    RenderBox box = anchorContext.findRenderObject();
